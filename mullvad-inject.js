...
exports.DaemonRpc = exports.ResponseParseError = exports.SubscriptionListener = exports.ConnectionObserver = void 0;
...
const grpcTypes = __importStar(require("./management_interface/management_interface_pb"));
const DAEMON_RPC_PATH = process.platform === 'win32' ? 'unix:////./pipe/Mullvad VPN' : 'unix:///var/run/mullvad-vpn';
...
const invalidErrorStateCause = new Error('VPN_PERMISSION_DENIED is not a valid error state cause on desktop');
...
class DaemonRpc {
    constructor(connectionObserver) {
        ...
        this.client = new management_interface_grpc_pb_1.ManagementServiceClient(DAEMON_RPC_PATH, grpc.credentials.createInsecure(), this.channelOptions());
    }
    get isConnected() {
    ...
    async submitVoucher(voucherCode) {
        try {
            const response = await this.callString(this.client.submitVoucher, voucherCode);
            const secondsAdded = ensureExists(response.getSecondsAdded(), "no 'secondsAdded' field in voucher response");
            const newExpiry = ensureExists(response.getNewExpiry(), "no 'newExpiry' field in voucher response")
                .toDate()
                .toISOString();
            return {
                type: 'success',
    ...
    async setRelaySettings(relaySettings) {
        if ('normal' in relaySettings) {
            ...
            await this.call(this.client.setRelaySettings, grpcRelaySettings);
        }
    }
    ...
    async setObfuscationSettings(obfuscationSettings) {
        ...
        switch (obfuscationSettings.selectedObfuscation) {
            case daemon_rpc_types_1.ObfuscationType.auto:
                grpcObfuscationSettings.setSelectedObfuscation(grpcTypes.ObfuscationSettings.SelectedObfuscation.AUTO);
                break;
            case daemon_rpc_types_1.ObfuscationType.off:
                grpcObfuscationSettings.setSelectedObfuscation(grpcTypes.ObfuscationSettings.SelectedObfuscation.OFF);
                break;
            case daemon_rpc_types_1.ObfuscationType.udp2tcp:
                grpcObfuscationSettings.setSelectedObfuscation(grpcTypes.ObfuscationSettings.SelectedObfuscation.UDP2TCP);
                break;
        }
        ...
        await this.call(this.client.setObfuscationSettings, grpcObfuscationSettings);
    }
    ...
    async setWireguardQuantumResistant(quantumResistant) {
        ...
        await this.call(this.client.setQuantumResistantTunnel, quantumResistantState);
    }
    ...
    subscribeDaemonEventListener(listener) {
        const call = this.isConnected && this.client.eventsListen(new empty_pb_js_1.Empty());
        if (!call) {
            throw noConnectionError;
        }
    ...
    async listDevices(accountToken) {
        try {
            const response = await this.callString(this.client.listDevices, accountToken);
    ...
    async createCustomList(name) {
        try {
            await this.callString(this.client.createCustomList, name);
        }
        catch (e) {
            const error = e;
            if (error.code === 6) {
    ...
    async deleteCustomList(id) {
        await this.callString(this.client.deleteCustomList, id);
    }
    async updateCustomList(customList) {
        try {
            await this.call(this.client.updateCustomList, convertToCustomList(customList));
        }
        catch (e) {
            const error = e;
            if (error.code === 6) {
    ...
    connectivityChangeCallback(timeoutErr) {
        ...
        logging_1.default.verbose(`GRPC Channel connectivity state changed to ${currentState}`);
        if (channel) {
            if (timeoutErr) {
    ...
    setChannelCallback(currentState) {
        ...
        if (currentState) {
            channel.watchConnectivityState(currentState, this.channelStateTimeout(), (error) => this.connectivityChangeCallback(error));
        }
    }
...
function convertFromBlockingError(error) {
    switch (error.type) {
        ...
        case grpcTypes.ErrorState.FirewallPolicyError.ErrorType.LOCKED: {
            ...
            return { type: daemon_rpc_types_1.FirewallPolicyErrorType.locked, pid, name };
        }
    }
}
...
function convertFromParameterError(error) {
    switch (error) {
        ...
        case grpcTypes.ErrorState.GenerationError.CUSTOM_TUNNEL_HOST_RESOLUTION_ERROR:
            return daemon_rpc_types_1.TunnelParameterError.customTunnelHostResolutionError;
    }
}
    ...
    if (state.tunnelEndpoint) {
        return {
            ...
            endpoint: {
                ...
                obfuscationEndpoint: state.tunnelEndpoint.obfuscation &&
                    convertFromObfuscationEndpoint(state.tunnelEndpoint.obfuscation),
                entryEndpoint: state.tunnelEndpoint.entryEndpoint &&
    ...
    if (relaySettings) {
        switch (relaySettings.getEndpointCase()) {
            ...
            case grpcTypes.RelaySettings.EndpointCase.NORMAL: {
                ...
                const location = (0, daemon_rpc_types_1.wrapConstraint)(locationConstraint);
                ...
                const wireguardConstraints = convertFromWireguardConstraints(normal.getWireguardConstraints());
                return {
                    normal: {
                        location,
                        tunnelProtocol,
...
function convertFromBridgeSettings(bridgeSettings) {
    ...
    if (normalSettings) {
        const locationConstraint = convertFromLocationConstraint(bridgeSettings.getNormal()?.getLocation());
        const location = (0, daemon_rpc_types_1.wrapConstraint)(locationConstraint);
...
function convertFromConnectionConfig(connectionConfig) {
    ...
    switch (connectionConfig.getConfigCase()) {
        ...
        case grpcTypes.ConnectionConfig.ConfigCase.WIREGUARD:
            return (connectionConfigObject.wireguard &&
                connectionConfigObject.wireguard.tunnel &&
                connectionConfigObject.wireguard.peer && {
                wireguard: {
                    ...connectionConfigObject.wireguard,
                    tunnel: {
                        privateKey: convertFromWireguardKey(connectionConfigObject.wireguard.tunnel.privateKey),
...
function convertFromLocationConstraint(location) {
...
function convertFromGeographicConstraint(location) {
...
function convertFromTunnelOptions(tunnelOptions) {
    return {
        ...
        wireguard: {
            ...
            quantumResistant: convertFromQuantumResistantState(tunnelOptions.wireguard?.quantumResistant?.state),
        },
        generic: {
            enableIpv6: tunnelOptions.generic.enableIpv6,
        },
        dns: {
            state: tunnelOptions.dnsOptions?.state === grpcTypes.DnsOptions.DnsState.CUSTOM
                ? 'custom'
                : 'default',
            defaultOptions: {
...
function convertFromWireguardConstraints(constraints) {
    ...
    if (entryLocation) {
        ...
        result.entryLocation = (0, daemon_rpc_types_1.wrapConstraint)(location);
    }
    return result;
}
...
function convertToRelayConstraints(constraints) {
    ...
    relayConstraints.setWireguardConstraints(convertToWireguardConstraints(constraints.wireguardConstraints));
    relayConstraints.setOpenvpnConstraints(convertToOpenVpnConstraints(constraints.openvpnConstraints));
...
function convertToGeographicConstraint(location) {
...
function convertToTunnelType(tunnelProtocol) {
...
function convertToWireguardConstraints(constraint) {
    if (constraint) {
        ...
        if (ipVersion) {
            const ipVersionProtocol = ipVersion === 'ipv4' ? grpcTypes.IpVersion.V4 : grpcTypes.IpVersion.V6;
...
function convertFromCustomListSettings(customListSettings) {
    return customListSettings ? convertFromCustomLists(customListSettings.getCustomListsList()) : [];
}
function convertFromCustomLists(customLists) {
    return customLists.map((list) => ({
        id: list.getId(),
        name: list.getName(),
        locations: list
            .getLocationsList()
            .map((location) => convertFromGeographicConstraint(location.toObject())),
    }));
}
function convertToCustomList(customList) {
